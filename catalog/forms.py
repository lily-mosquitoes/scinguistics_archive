...
class LessonCreateForm(forms.ModelForm):
    recording_processing_link = forms.CharField(
        required=False, max_length=300, help_text="Input a CRAIG or GIARC link"
    )
    form_date_and_time = forms.CharField(
        required=False,
        max_length=50,
        help_text="Input the date and time of the lesson as an ISO timestamp string (e.g.: 2021-06-15T00:04:22.468Z) -> this is in info.txt",
    )
    form_recording_file = forms.FileField(
        required=False, help_text="Upload the lesson recording file"
    )
    ...
    def clean(self):
        ...
        if not req1 and not req2:
            raise ValidationError(
                "Please input either the recording link or the file + date and time"
            )
        ...
        if req2 and not req3:
            raise ValidationError(
                "Please input the lesson date and time when directly upploading the recording file"
            )
    ...
    def clean_recording_processing_link(self):
        ...
        if uploaded_file == None:
            ...
            raise ValidationError(
                "ONLY CHOOSE ONE OPTION: INPUT CRAIG/GIARC LINK OR UPLOAD A FILE"
            )
        ...
        if "craig" in base_url:
            ...
            rec_key = query["key"][0]
            base_url = (
                f"{parsed.scheme}://{parsed.netloc}/api/rec/{rec_n}?key={rec_key}"
            )
    ...
    def clean_form_date_and_time(self):
        ...
        try:
            form_date_and_time = datetime.fromisoformat(
